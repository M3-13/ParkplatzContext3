...
4. **Tests are part of done.** Every ticket includes writing tests AND making
   them pass. `the stack's own build and test command` must exit 0 before you stop.
...
## No CI pipeline for this stack

The office ships CI templates only for the stacks whose commands run on a bare Linux
runner; this project is not one of them, so there is NO pipeline file in the repo and
`git push` triggers nothing. Do NOT write one yourself — `.github/workflows/` and
`.gitlab-ci.yml` stay office-owned and empty.

What stands in for the pipeline: after your push the office builds and runs the tree
on THIS machine with the stack's own toolchain — the build must succeed and every test
must pass there. Run `the stack's own build and test command` yourself before you finish; that run is the
verification this merge gets, so what you did not run, nobody ran.
